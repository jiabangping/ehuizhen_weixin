...
public class DoctorAuditDao {
	...
	public DoctorAuditModel getDoctorAuditByKey(final String field, final Object v) throws Exception {
		try {
			DoctorAuditModel model = hibernateTemplate.execute(new HibernateCallback<DoctorAuditModel>() {
				...
				public DoctorAuditModel doInHibernate(Session session) throws HibernateException {
					Query query = session
							.createQuery("FROM DoctorAuditModel WHERE "+field+" =:"+field);
					query.setParameter(field, v);
					return (DoctorAuditModel) query.uniqueResult();
				}
			});
			return model;
	 ...
	 * @return
	 */
	public DoctorAuditModel getDoctorAuditByWxId(final String wxId) throws Exception {
		try {
			DoctorAuditModel model = hibernateTemplate.execute(new HibernateCallback<DoctorAuditModel>() {
				...
				public DoctorAuditModel doInHibernate(Session session) throws HibernateException {
					Query query = session
							.createQuery("FROM DoctorAuditModel WHERE wxOpenId =:wxOpenId ");
					query.setParameter("wxOpenId", wxId);
					return (DoctorAuditModel) query.uniqueResult();
				}
			});
			return model;
	...
	@Transactional(isolation=Isolation.REPEATABLE_READ,propagation=Propagation.REQUIRED)
	public boolean delete(DoctorAuditModel model) throws Exception {
		try{
